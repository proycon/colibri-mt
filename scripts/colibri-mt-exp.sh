...
if [ "$RUN" = "1" ]; then
    ...
    if [ "$MOSESONLY" = "1" ]; then
        ...
        if [[ $MERT -ge 1 ]]; then
            ...
            if [ ! -d "$CLASSIFIERDIR/$CLASSIFIERSUBDIR/$DECODEMERTDIR" ] || [ ! -f "$CLASSIFIERDIR/$CLASSIFIERSUBDIR/$DECODEMERTDIR/mert-work-${MERT}/moses.ini" ] || [ ! -f "$CLASSIFIERDIR/$CLASSIFIERSUBDIR/test.txt" ]; then
